<main id="main">
  ...
  <section id="breadcrumbs" class="breadcrumbs">
    <div class="container">
      ...
      <div class="d-flex justify-content-between align-items-center">
        <h2>Reservasi & Take Away</h2>
        <ol>
          <li><a href="<?= base_url() ?>">Home</a></li>
          <li>Reservasi & Take Away</li>
        </ol>
      </div>
  ...
  <section id="contact-us" class="contact-us">
    <div class="container">
      <div class="row">
        <div class="col-lg-6">
          <h3>Formulir Reservasi dan Take Away</h3>
          ...
          <div class="form-group mb-2">
            <label>Pilih Meja Yang Ingin Direservasi / Take Away</label>
          ...
          <div class="form-group mb-2" id="catatan_wrapper" style="display:none;">
            <label>Catatan Reservasi</label>
            <small class="form-text" style="color: red;">*Wajib Diisi Jika Reservasi</small>
            <input type="text" class="form-control" id="note_reservasi" name="note_reservasi" placeholder="Misalnya: Hari Senin, Jam 20.00" required>
        ...
        <div class="col-lg-6">
          ...
          <div class="row">
            <div class="col-lg-12">
              <form action="<?= base_url() ?>home/tambahPesanan" method="POST" onsubmit="return validateBooking();">
                <span id="daftar">
                  <div id="keterangan_nama_nomor_hp"></div>
                  <div id="keterangan_tanggal_dipilih"></div>
                  <div id="keterangan_meja_dipilih"></div>
                  <div id="keterangan_note_reservasi"></div>
                  <div class="row">
                    <h4 id="judul_menu">
                      <hr>
                      Menu Yang Dipesan
                    </h4>
                    <div class="col-lg-6" id="daftar_menu_dipesan"></div>
                    <div class="col-lg-6" id="daftar_harga_dipesan"></div>
                    <div class="col-lg-12 mt-1" id="total_harga"></div>
                  </div>
                </span>
...
<script>
  ...
  document.getElementById('id_meja').addEventListener('change', function() {
    ...
    const wrapper = document.getElementById('catatan_wrapper');
    const noteDetail = document.getElementById('keterangan_note_reservasi');

    // Tampilkan kolom catatan
    wrapper.style.display = "";

    if (selected.startsWith("TAKEAWAY")) {
      wrapper.innerHTML = `
      <label>Catatan Take Away (Opsional)</label>
      <textarea class="form-control" id="note_reservasi" name="note_reservasi" placeholder="Misalnya: Jangan Terlalu Pedas..."></textarea>
    `;
      document.getElementById('note_reservasi').addEventListener('input', function() {
        const note = this.value.trim();
        noteDetail.innerHTML = `<b>Catatan Take Away</b> = ${note || '-'}
        <input type="hidden" name="hidden_note_reservasi" value="${note || '-'}">`;
      });
      // Default awal kosong = tampil "-"
      noteDetail.innerHTML = `<b>Catatan Take Away</b> = -
      <input type="hidden" name="hidden_note_reservasi" value="-">`;
    } else {
      wrapper.innerHTML = `
      <label>Catatan Reservasi</label>
      <small class="form-text" style="color: red;">*Wajib Diisi Jika Reservasi</small>
    ...
    `;
      document.getElementById('note_reservasi').addEventListener('input', function() {
        const note = this.value.trim();
        noteDetail.innerHTML = `<b>Catatan Reservasi</b> = ${note}
        <input type="hidden" name="hidden_note_reservasi" value="${note}">`;
      });
      noteDetail.innerHTML = ""; // reset kalau baru pilih meja
    }
  });

  function validateBooking() {
    const idMeja = document.querySelector('input[name="hidden_id_meja"]').value;
    const noteInput = document.getElementById('note_reservasi');

    // Kalau reservasi meja (bukan TAKEAWAY)
    if (idMeja !== "TAKEAWAY") {
      if (!noteInput || noteInput.value.trim() === "") {
        alert("Catatan Reservasi wajib diisi jika memilih meja!");
        noteInput.focus();
  ...
  function pilih_tanggal(tanggal) {
    ...
    $.ajax({
      ...
      dataType: 'json',
      success: (hasil) => {
        let isi = `<option disabled selected value="">Pilih Meja Yang Ingin Direservasi/Take Away</option>`;
        // Tambahkan opsi Take Away permanen
        isi += `<option value="TAKEAWAY|0">Take Away (Bungkus / Dibawa Pulang)</option>`;
        hasil.forEach(function(item) {
          isi +=
    ...
    `;
    let isinyatanggal = `
    <b>Tanggal Pemesanan</b> =  ${formatDate(tanggal)}
  ...
  function tambah_meja(datameja) {
    ...
    if (myArr[0] === "TAKEAWAY") {
      isinya = `
      <b>Pesanan</b> = Take Away (Bungkus / Dibawa Pulang)
      <input type="hidden" name="hidden_id_meja" value="TAKEAWAY">
    `;
    } else {
      isinya = `
  ...
  // Event untuk menampilkan catatan reservasi di detail pesanan
  document.getElementById('note_reservasi').addEventListener('input', function() {
    const note = this.value.trim();
    const noteDetail = document.getElementById('keterangan_note_reservasi');
